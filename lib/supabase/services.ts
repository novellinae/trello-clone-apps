import { Board, Column, Task } from "./module";
...
export const boardService = {
    ...
    async createBoards(
        ...
        const { data, error } = await supabase
            .from("boards")
            .insert(board)
            .select() //biar ngereturn view hasil insert
            .single() //biar cuman 1 items
    ...
    async updateBoard(
        ...
        const { data, error } = await supabase
            ...
            .eq("id", boardId)
            .select() //biar ngereturn view hasil update
            .single() //biar cuman 1 items
...
export const boardDataService = {
    ...
    async createBoardWithDefaultColumns( 
        ...
        await Promise.all(
            defaultColumns.map(
                (column) => columnService.createColumn(supabase, {
                    ...column, 
                    board_id: board.id,
                    user_id: boardData.userId,})
            )
